...
impl Board {
    pub fn build_board(reader: &mut Read) -> Result<Board> {
        ...
        let board_json: Json =
            try!(Json::from_reader(reader).map_err(|_| "Unable to create JSON reader".to_string()));
    ...
    fn parse_rooms(&mut self, json: &Json) -> Result<()> {
        ...
        // Parse each room
        let rooms: &Vec<Json> = try!(json_rooms
            .as_array()
    ...
    fn parse_halls(&mut self, json: &Json) -> Result<()> {
        ...
        // Parse each hall
        let halls: &Vec<Json> = try!(json_halls
            .as_array()
        ...
        for h in halls {
            ...
            let a: usize = try!(h[0].as_u64().ok_or("Unable to parse hall 0".to_string())) as usize;
            let b: usize = try!(h[1].as_u64().ok_or("Unable to parse hall 1".to_string())) as usize;
            hall.left = self.rooms[a].clone();
            hall.right = self.rooms[b].clone();

            let rc_hall = Rc::new(hall);
            // Add hall links to rooms

            self.rooms[a].borrow_mut().halls.push(rc_hall.clone());
            self.rooms[b].borrow_mut().halls.push(rc_hall.clone());
        }
        Ok(())
    }
